...
export const Benefits = (props: Readonly<BenefitsProps>) => {
  ...
  if (!data || typeof data !== 'object') {
    console.error('Invalid data prop passed to Benefits component:', data);
    return null;
  }

  if (!data.image || typeof data.image !== 'object') {
    console.error('Invalid or missing image in data prop:', data.image);
    return null;
  }
        ...
        <div>
          ...
          <div className="w-full mt-5">
            {data.bullets && data.bullets.map((item, index) => (
              <Benefit key={index} titleKey={item.titleKey} icon={item.icon}>
                {t(item.descKey)}
              </Benefit>
            ))}
          </div>
...
function Benefit(props: any) {
  ...
  if (!props.icon || !React.isValidElement(props.icon)) {
    console.error('Invalid icon prop passed to Benefit component:', props.icon);
    return null;
  }
